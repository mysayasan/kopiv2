...
const SHOT_ALT = [
  ...
  '带截图和确认操作的统一通知信息流',
  '事件分析仪表板：KPI、随时间变化的检测图表、类别环形图以及按摄像头的细分',
  '通知目的地——webhook、Telegram 和 MQTT，支持按类别订阅、字段开关和模板',
  '安全设置——静态加密、带恢复托管的密钥保护、可移植备份以及安全擦除',
];
...
export default {
  ...
  showcase: {
    ...
    lead: '实时多摄像头网格、绘制在真实画面上的检测规则、统一的告警信息流、事件分析、通知路由以及本机安全——每个界面都在浏览器中。',
    tabs: { live: '实时画面', rules: '检测规则', notifications: '通知', dashboard: '仪表板', destinations: '随处通知', security: '安全' },
