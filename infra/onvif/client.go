...
// StreamOption is one selectable ONVIF media profile and its RTSP URI.
...
// StreamOptionsResult contains all selectable RTSP streams exposed by ONVIF media profiles.
type StreamOptionsResult struct {
	MediaXAddr            string         `json:"mediaXAddr"`
	SelectedProfileToken  string         `json:"selectedProfileToken"`
	PreferredProfileToken string         `json:"preferredProfileToken"`
...
// GetStreamOptions resolves every ONVIF media profile to a selectable RTSP stream option.
func (c *Client) GetStreamOptions(ctx context.Context, req StreamURIRequest) (*StreamOptionsResult, error) {
	deviceURL, err := NormalizeDeviceServiceURL(req.DeviceServiceURL)
	if err != nil {
		return nil, err
	}
	...
	preferredToken, err := ParsePreferredProfileToken(body)
	if err != nil {
		return nil, err
	}
	selectedToken := strings.TrimSpace(req.ProfileToken)
	if selectedToken == "" {
		selectedToken = strings.TrimSpace(preferredToken)
	}

	options := make([]StreamOption, 0, len(profiles))
	var failures []string
	for _, profile := range profiles {
		rtspURL, streamErr := c.getStreamURI(ctx, mediaURL, profile.Token, req.Credentials)
		if streamErr != nil {
			failures = append(failures, fmt.Sprintf("%s: %v", profile.Token, streamErr))
			continue
		}
		options = append(options, StreamOption{
			...
			RTSPURL:      rtspURL,
			Preferred:    profile.Token == preferredToken,
			Selected:     profile.Token == selectedToken,
		})
	}
	if len(options) == 0 {
		if len(failures) > 0 {
			return nil, fmt.Errorf("ONVIF stream URI not found for any media profile: %s", strings.Join(failures, "; "))
		}
		return nil, errors.New("ONVIF media profile token not found")
	}

	return &StreamOptionsResult{
		MediaXAddr:            mediaURL,
		SelectedProfileToken:  selectedToken,
		PreferredProfileToken: strings.TrimSpace(preferredToken),
		Options:               options,
	}, nil
}
