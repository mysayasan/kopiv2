...
func TestGetStreamOptionsResolvesEveryMediaProfile(t *testing.T) {
	var serverURL string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
  ...
  <s:Body>
    <trt:GetProfilesResponse xmlns:trt="http://www.onvif.org/ver10/media/wsdl" xmlns:tt="http://www.onvif.org/ver10/schema">
      ...
      <trt:Profiles token="sub">
		...
		case strings.Contains(payload, "GetStreamUri") && strings.Contains(payload, ">main<"):
			_, _ = w.Write([]byte(`<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
  <s:Body>
    <trt:GetStreamUriResponse xmlns:trt="http://www.onvif.org/ver10/media/wsdl">
      <trt:MediaUri><tt:Uri xmlns:tt="http://www.onvif.org/ver10/schema">rtsp://192.168.1.40/stream1</tt:Uri></trt:MediaUri>
    </trt:GetStreamUriResponse>
  </s:Body>
</s:Envelope>`))
		case strings.Contains(payload, "GetStreamUri") && strings.Contains(payload, ">sub<"):
			_, _ = w.Write([]byte(`<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope">
  <s:Body>
    <trt:GetStreamUriResponse xmlns:trt="http://www.onvif.org/ver10/media/wsdl">
      <trt:MediaUri><tt:Uri xmlns:tt="http://www.onvif.org/ver10/schema">rtsp://192.168.1.40/stream2</tt:Uri></trt:MediaUri>
    </trt:GetStreamUriResponse>
  </s:Body>
</s:Envelope>`))
		default:
	...
	client := &Client{HTTPClient: server.Client()}
	res, err := client.GetStreamOptions(context.Background(), StreamURIRequest{
		DeviceServiceURL: server.URL,
		ProfileToken:     "main",
	})
	if err != nil {
		t.Fatalf("GetStreamOptions() error = %v", err)
	}
	if res.MediaXAddr != server.URL+"/media" || res.SelectedProfileToken != "main" || res.PreferredProfileToken != "sub" {
		t.Fatalf("stream options metadata = %+v", res)
	}
	if len(res.Options) != 2 {
		t.Fatalf("option count = %d, want 2", len(res.Options))
	}
	if res.Options[0].ProfileToken != "main" || res.Options[0].RTSPURL != "rtsp://192.168.1.40/stream1" || !res.Options[0].Selected {
		t.Fatalf("main option = %+v", res.Options[0])
	}
	if res.Options[1].ProfileToken != "sub" || res.Options[1].RTSPURL != "rtsp://192.168.1.40/stream2" || !res.Options[1].Preferred {
		t.Fatalf("sub option = %+v", res.Options[1])
	}
}
...
func TestParsePreferredProfileTokenChoosesLowerResolutionProfile(t *testing.T) {
  ...
  <s:Body>
	...
	if len(parsed) != 2 {
		t.Fatalf("profile count = %d, want 2", len(parsed))
	}
	if parsed[0].Token != "main" || parsed[0].Name != "MainStream" || parsed[0].Encoding != "H265" || parsed[0].Width != 3840 || parsed[0].Height != 2160 {
		t.Fatalf("main profile = %+v", parsed[0])
	}
	if parsed[1].Token != "sub" || parsed[1].Name != "SubStream" || parsed[1].Encoding != "H264" || parsed[1].Width != 640 || parsed[1].Height != 360 {
		t.Fatalf("sub profile = %+v", parsed[1])
	}
}
