...
- `apps/myseliasan`: relying control-plane app for `mymatasan`, authenticated through `myidsan`. Now also ships its own smaller, fleet-scoped **built-in user manual** (6 articles × 4 languages, `/api/manual`) on the same shared `domain/shared/manual` library — phase 2 of the suite-wide manual, see "Built-in user manual" in `apps/myseliasan/README.md`.
