...
- `apps/mypintusan`: physical access-control appliance (door badge readers over OSDP). **P1 built and now runnable, with a frontend and a first-run wizard; `myseliasan` fleet adoption (P2) is wired.** Polls RS-485/TCP OSDP readers (`infra/access/osdp`, re-dialling with backoff if the transport dies), decides badge-in grants against a SQLite-backed access log with schedules/holidays/lockdown/duress, and fires the door strike through one audited chokepoint. A React SPA (Doors/People/Activity/Readers/Settings) and a welcome-to-first-badge setup wizard are built on the shared `frontend/shared` module, and `config.json`'s access/bus settings are now a first-boot seed only — an operator edits them afterwards through `GET`/`PUT /api/settings/access`. mypintusan is now adoptable into a `myseliasan` fleet exactly like `mymatasan`/`myiotsan` (`fleetnode.KindDoor`, two channels — no media, a door has no video): the control plane discovers, adopts and manages a door controller over the node-dialed control channel, embeds its real Doors/People/Activity/Readers/Settings pages, and correlates its badge decisions against camera and sensor events (the flagship rule — motion AND a door opening AND no badge accepted — is now literally true across all three appliance kinds). Groups/schedules/grants screens, reader onboarding beyond the wizard's single reader, serial bus transport, and myiotsan door-contact/relay bindings are not built yet — see `docs/MYPINTUSAN_DATA_MODEL.md` and `docs/MYPINTUSAN_OSDP_PLAN.md`.
