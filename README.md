...
- `apps/mypintusan`: physical access-control appliance (door badge readers over OSDP). **P1 built and now runnable, API only — no frontend yet.** Polls RS-485/TCP OSDP readers (`infra/access/osdp`, re-dialling with backoff if the transport dies), decides badge-in grants against a SQLite-backed access log with schedules/holidays/lockdown/duress, and fires the door strike through one audited chokepoint. Serial bus transport, myiotsan door-contact/relay bindings, and `myseliasan` fleet adoption are not built yet — see `docs/MYPINTUSAN_DATA_MODEL.md` and `docs/MYPINTUSAN_OSDP_PLAN.md`.
