...
- `apps/myseliasan`: relying control-plane app for `mymatasan`, authenticated through `myidsan`. Now also ships its own **built-in user manual** (22 articles × 4 languages, `/api/manual`) on the same shared `domain/shared/manual` library — phase 2 of the suite-wide manual, see "Built-in user manual" in `apps/myseliasan/README.md`. Its "ask the fleet" AI chat can also ground answers in that manual (and mymatasan's) via a lexical retrieval library (`domain/shared/manual/retrieval`), so a how-to question gets a citeable answer even with the optional language model switched off.
