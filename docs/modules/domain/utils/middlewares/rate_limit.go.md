...
- Unauthenticated request identity for bucketing is resolved by `(*RateLimitMidware).clientIP`, which now delegates to the shared `ClientIP` in `clientip.go` (`domain/utils/middlewares/clientip.go.md`): it honors `X-Forwarded-For` / `X-Real-IP` only when the direct TCP peer (`r.RemoteAddr`) matches a configured `RateLimitConfig.TrustedProxies` entry (IP or CIDR, parsed by the shared `ParseTrustedProxies`); otherwise it uses the peer address itself. `TrustedProxies` defaults to empty (trust none), so a directly internet-exposed instance cannot have its rate-limit/login-lockout bucket key spoofed by a forged forwarding header — a client behind an untrusted/unset proxy is bucketed by the proxy's own peer address (shared across all its clients), which is the safe fallback. Deployments that terminate TLS at a real reverse proxy must set `rateLimit.trustedProxies` to that proxy's address(es) to get accurate per-client buckets again. This resolution logic was previously private to this file; it moved to `clientip.go` so myidsan's audit trail (`apps/myidsan/apis/audit.go.md`) shares the identical trusted-proxy answer rather than forking its own copy — see that module's doc for why a fork here is a security-relevant bug, not a style preference.
