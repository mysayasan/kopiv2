...
- Group uses auth middleware + accessrbac middleware (`AccessSessionMidware`).
...
- POST/PUT accept `metadata` as JSON text for menu and presentation customization; API authorization comes from the accessrbac permission matrix (`/api/access-rbac`), not from this endpoint catalog.
