...
- `dispatch`'s `FrameHello` case (W2-5, F-07) — the version a node reports on connect used to be logged and thrown away; it is now also handed to `INodeRegistry.RecordVersion(ctx, nodeID, version)` (best-effort, off the read loop in its own goroutine with a 10s timeout, so a database hiccup can never cost the connection itself). This is the control plane's only durable record of what a node is actually running, and it is what a staged version rollout (`services/fleet_rollout.go.md`) judges a ring against — a `nil`/unset `registry` or an empty reported version is simply skipped.
- `dispatch`'s `FrameHello` case (W2-6, F-11) — also reads the new `frame.Dropped`: a node admitting it could not forward `N` events while it was disconnected is logged (`control: node %s reports %d event(s) dropped while disconnected`) and, when `frame.Dropped > 0`, handed to the optional `onDropReport(nodeID, dropped)` callback (in its own goroutine, off the read loop, same pattern as `onConnect`/`onDisconnect`). This is exactly the number the reconnect notification replay (`app.go.md`'s "Replay on reconnect") is expected to recover, so recording it is what makes that expectation checkable rather than merely believed.
- `SetDropReportHandler(fn func(nodeID string, dropped int64))` — registers the drop-report callback. Set once at startup, before `Run`; nil is a no-op. `app.go` wires it to increment `services.MetricNodeEventsDroppedTotal` and publish an info-level "Node reconnected after dropping events" notification into the unified feed.
