...
passed as `DeploymentEnv.ExtraChecks` (below). The env also supplies `LoginGuardEnabled`/`LoginGuardShared` from the guard above, feeding the shared `loginLockout` blocker row. Reports `agent.llm.mode == "sidecar"` as a
...
- `20260908-01-node-no-fixed-location` — add `managed_node.no_fixed_location` (`BOOLEAN`, an
  operator's explicit "this appliance deliberately has no place on any plan" — see
  `entities/managed_node.go.md`'s `NoFixedLocation`), backfilled to `false`. Same NULL-safety
  reason as `auto_renew`: the entity's field is a non-pointer `bool` and cannot scan a `NULL` a
  bare `ADD COLUMN` would leave on every existing row, and `false` is exactly what "nobody has
  made that decision yet" means for a pre-existing node.
...
- Builds the **failed-login lockout** — `sharedapis.NewLoginGuardFor(deps.Config.LoginSecurity.Effective())`,
  attached to the shared cache with `WithSharedStore(deps.Cache, "myseliasan")` when
  `sharedservices.IsSharedCacheProvider(deps.Config.Cache.Provider)` — and threads it, together with
  `auditService`, into `NewAuthApi`. This app previously had **no lockout at all**: `local-login`
  checked the password and returned, so the only brute-force control was the generic rate limiter
  (30/min, per source, per path, per instance, refilling forever). See `apis/auth.go.md`. The
  namespace matters: myidsan and myseliasan are routinely pointed at one Redis, and a lockout on
  the identity server is not a lockout on the fleet console.
- Registers auth/session routes (`NewAuthApi`, `NewSessionApi`).
- Builds `IPushService` (`services.NewPushService(deps.Db, secretCipher, auditService, logf)`) and mounts `NewPushApi` at `/api/push` — mobile push, W3-9. The service is registered on the notification hub as an ordinary channel **with no filter**, because the severity floor is a property of the DEVICE, not of the install: the phone in somebody's pocket at 3am and the laptop on their desk want different thresholds, and one install-wide filter here would force the stricter one on everybody. It is the only channel on this appliance that talks to a THIRD PARTY (a browser vendor's push service), which on intranet deployments cannot be reached at all — so it reports its capability from real delivery attempts rather than from configuration. See `services/push.go.md`.
...
- Registers node-management routes (`NewNodesApi`, now passed a `logf` closure so a failed adopt logs its raw cause server-side even though the client gets a friendlier message), which now also exposes `PUT /api/nodes/{id}/position` for the geographic map, `PUT /api/nodes/{id}/no-fixed-location` for the digital-twin "this appliance deliberately has no place on any plan" waiver (replacing the removed `PUT /api/nodes/{id}/building` — a box's place is now set only by pinning its own marker on a plan, see `services/sites.go.md`'s `SetNodeSiteBinder`), and `GET /api/nodes/unrecognized` + `POST /api/nodes/{id}/block`/`forget` for stranded-node visibility (see `apis/nodes.go.md`).
...
- `FleetEventNodeRecovered` → `Info` "Node back online". **This is the all-clear, and until W3-9 it could not fire on a real fleet**: the liveness sweep emits it on the lost → online edge, but a node normally comes back by DIALLING IN, and `AcceptControlConn` marks it online between sweeps — so by the time the sweep looked, the previous status was already `online`. The registry now raises the event from the dial-in and enrolment paths as well (see `services/node_registry.go.md`), edge-triggered so a reconnect on an already-online node stays silent. Found by the W3-9 push bench, which took a node down for real and counted what reached a device.
