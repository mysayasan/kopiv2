...
| `Fov` | The field-of-view spread in degrees. `0` means "no arc" — a plain marker, the default for a node/sensor placement. A camera placement defaults to `70` on drop (`AddPlacement`); the operator then aims it (`Heading`) and widens/narrows it (`Fov`) via the on-marker FOV handles in the floor editor (`floor_editor.js`, opened from the plan workspace's own browser tab — `components/plan_workspace.js`). |
