...
| `GET` | `/api/notifications/baseline` | Fleet-wide expected-activity band for each bucket of the events-over-time chart, matching its granularity (`?bucket=hour\|day`). Rollup-backed (`domain/notification.RollupMaintainer` — see `services/rollup_cursor.go.md`); empty until the first sweep lands. `?from=`/`?to=`/`?tzOffset=` same contract as `stats`. **Fleet-wide only, no `?cameraId=` filter**: the control plane's feed keys events by source *node*, and origin camera ids collide across nodes, so the per-camera narrowing mymatasan offers would silently merge unrelated cameras here. Backs the Dashboard/Insight chart's expected band and the AI digest's `baseline_spike`/`baseline_quiet` findings (`services/agent_findings.go.md`). Same endpoint mymatasan already exposes, now on myseliasan for the first time. |
