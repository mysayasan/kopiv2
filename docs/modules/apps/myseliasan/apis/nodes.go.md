...
| PUT | `/api/nodes/{id}/no-fixed-location` | Record (`noFixedLocation: true`) or withdraw (`false`) an operator's decision that an appliance has no place on any plan — a colo recorder, a hosted hub: `{noFixedLocation}`. Calls `INodeRegistry.SetNoFixedLocation`, which also clears `SiteId`/`MapPlaced` when set. **Replaces the removed `PUT /api/nodes/{id}/building`** — there is no longer a route to assign a building directly; that now happens exactly one way, by pinning the appliance's own marker on a plan (`ISiteService.AddPlacement`, see `apis/sites.go.md` and `services/sites.go.md`). This route is the other half: the only way to answer "nowhere, on purpose" instead of leaving the question unanswered forever in the fleet map's "not placed yet" tray. Returns the updated `ManagedNode`; 400 on an unknown node or bad body. |
| PUT | `/api/nodes/{id}/auto-renew` | Toggle a node's certificate auto-renew gate: `{autoRenew: bool}`. Off (the default for a newly adopted node) lets the node's certificate lapse when it expires — the node's automatic re-enrollment attempt is refused (`services.ErrRenewNotAuthorized`) until an operator turns this on. No certificate is issued by this call. Returns the updated `ManagedNode`; 400 on an unknown node or bad body. Audited as `node.auto_renew`. |
| GET | `/api/nodes/availability` | SLA availability over a past window (`from`/`to` unix seconds, or `days`, default the trailing 30), per node, per site and per calendar month. See `services/node_availability.go.md`. A read of the same fleet data the list returns, so it sits under the same page grant; registered before the `/{id}` routes as a distinct literal. **A caller must read `hasData` before `availability`** — 0 with `hasData` false means "unknown", and printing it as 0% reports a healthy node nobody happened to observe as one that was never up. Returns an error rather than an empty report when history is unavailable, since an empty report reads as a fleet with no downtime. |
...
| POST | `/api/nodes/enroll` | Pairing token | Node-initiated CSR signing. Body: `{"nodeId","token","csr"}`. The token is the one issued to the node at adoption (renewal needs no claim code — the same pairing token is reused). Returns `{"nodeCert":"<PEM>","caRoot":"<PEM>"}` on success; 403 on unknown node or token mismatch; 451 if the node is revoked, **or if this is a renewal (the node already holds a previously issued cert) and the node's `AutoRenew` gate is off** (`services.ErrRenewNotAuthorized`, mapped the same non-5xx way as revoked). Initial enrollment right after adoption is always allowed regardless of `AutoRenew`. |
...
| `node.auto_renew` | `setAutoRenew` succeeds or fails | node id | Success detail notes whether auto-renew was enabled or disabled; `Metadata: {autoRenew: bool}`. |
