...
| PUT | `/api/nodes/{id}/building` | Assign (`siteId > 0`) or clear (`siteId: 0`) the building this appliance **resides in**: `{siteId}`. `INodeRegistry.UpdateNodeSite` also clears `MapPlaced` when assigning a building, since a building-resident node is represented by its building's marker on the geo map, not its own pin — the building-first digital-twin model (see `apis/sites.go.md`). Clearing the building leaves the node building-less (placeable standalone again); it is not automatically re-placed. Returns the updated `ManagedNode`; 400 on an unknown node or bad body. |
