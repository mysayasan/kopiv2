...
- `NewRecordingApi` also takes a `metrics telemetry.Metrics` argument, carried on `recordingApi.metrics` and threaded into the same `RecorderConfig` literal (`Metrics: a.metrics`) so a hot-reloaded recorder keeps counting `kopiv2_recording_ffmpeg_restarts_total`/`kopiv2_recording_segment_finalize_total` instead of silently losing them the moment an operator saves a recording setting — exactly the class of bug `shredPasses` above documents.
- **This handler reconstructs a `RecorderConfig` field-by-field, duplicating the one built in `app.go`.** Every new `RecorderConfig` field has to be remembered in both places or it is silently dropped here; treat adding a field to `RecorderConfig` as a two-site change until a shared builder replaces this duplication.
