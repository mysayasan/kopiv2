...
- `POST /api/onvif/devices/{id}/stream-options`: list every ONVIF media profile with profile token, codec, resolution, RTSP URI, and preferred/selected markers.
- `POST /api/onvif/devices/{id}/stream-uri`: resolve and save the preferred or selected saved-device profile to an RTSP URI, immediately probing VIGI-style fallback stream paths before persisting the working URL.
...
- `POST /api/onvif/devices/{id}/live-view`: resolve the ONVIF snapshot URI for browser live view and preserve the selected RTSP profile.
...
- `GET /api/onvif/devices/{id}/live.mjpeg`: stream RTSP or snapshot frames as multipart MJPEG fallback; `preferSnapshot=1` tries snapshot frames first, while `source=snapshot` forces snapshot-only output. RTSP conversion uses the runtime decoder ffmpeg settings.
...
- If WebRTC is disabled by runtime settings, or the saved RTSP metadata shows non-H264 video, the frontend uses `live.mjpeg` with snapshot preference directly.
- The frontend uses `stream-options` before `stream-uri` so operators can choose camera stream1/stream2 instead of always accepting the preferred profile.
