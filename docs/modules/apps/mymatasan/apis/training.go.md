...
| `POST`   | `/api/training/stock-model`                     | Set the base model (`{model}`); known names are downloaded, custom is a local .pt path; reloads the worker. A known name matches either the filename (`yolo11x.pt`) or the bare variant (`yolo11x`), via `canonicalStockModel` in `training_runner.go`; anything else is treated as a local path. Both pickers send the filename the `GET` lists — the setup wizard's dropdown used to strip the `.pt`, which made every stock model fail with `model file not found`, so `canonicalStockModel` accepts the bare form too rather than relying on callers getting the suffix right. |
