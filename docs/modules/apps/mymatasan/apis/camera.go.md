...
| GET    | `/api/cameras/{id}/talk` | `talkCapability` | Returns `TalkCapabilityResult` (`{supported, transport, detail}`) — whether the camera exposes an ONVIF audio backchannel for two-way audio (talk-back). Cached server-side (10 min TTL); backs the live-view mic button's show/hide. |
