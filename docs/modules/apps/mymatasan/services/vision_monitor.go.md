...
## Appearance capture gate (W3-2)

`sampleCamera` also computes `wantAppearance = m.metadata != nil && m.metadata.IsAppearanceEnabled(cameraID)` and sets `frame.WantAppearance = wantAppearance` after capture. Unlike LPR/face, this is a **per-camera** setting rather than a per-rule one — it describes every recorded person/vehicle for later search, not something any single alert rule asks for — and it deliberately does **not** join `wantLPR || wantFace` to force the high-resolution `CaptureForLPR` grab: LPR/face are targeted stages that run only while a rule is active, while this one runs on every sampled frame of an enabled camera, so forcing high-res capture for it would quietly multiply the baseline cost of the whole sampling loop. The worker's own minimum crop-size floor (`APPEARANCE_MIN_BOX_FRACTION` in `apps/mymatasan/ai/yolo_worker.py.md`) already discards figures too small to describe, so a modest capture resolution yields fewer descriptors, not bad ones. See `services/metadata_recorder.go.md`'s *Appearance descriptors* section for `IsAppearanceEnabled` and how the resulting vector reaches storage.
