...
- Persist `AppearanceEnabled` (W3-2) as `req.AppearanceEnabled && req.Enabled` — unlike `MetadataEnabled`, it is **not** forced to track `Enabled`: appearance is real compute (a forward pass per person/vehicle in every sampled frame) that metadata is not (metadata reuses inference the detector already ran), so switching recording on must not silently switch appearance on too. It is only ever forced *off* alongside recording, never forced *on* — it stays exactly where the operator left it otherwise.
...
- **Every purge below consults the case-file footage hold** (`case_hold.go.md`): footage an
  OPEN case points at is not deleted, and the loops advance a `heldSoFar` offset so a held row
  at the front of the oldest-first window does not stop everything behind it. A hold that
  cannot be read means everything is held — it fails closed. The one exception is
  `PurgeAllForCamera`, below.
- `PurgeAllForCamera(ctx, cameraId)`: delete EVERY segment for one camera regardless of expiry
  **or of any case hold** — reads oldest-first batches of 500 (deleting each batch before the next read so the window advances without offset drift), securely removing each file via `recording.SecureRemove`. Returns the count deleted. This is the **camera-delete cascade's** purge: the camera is going away, and footage held by a case nobody could then find or release would be held forever.
- `PurgeCameraFootage(ctx, cameraId)`: the operator-facing **"Purge now"** (`POST /api/recording/purge-camera`, paired with `visionService.PurgeAlertsForCamera` and the observation purge). The same destruction, except footage an OPEN case is holding is KEPT and reported back in a `PurgeFootageResult{Deleted, Kept, Reason}` — `Kept > 0` is the purge working, and the reason names the case, because a purge that silently leaves footage behind is one nobody trusts again. Two methods rather than one with a flag: the difference is a policy decision and it should be visible at the call site.
