...
- `INotificationService` (selected)
  ...
  - `ListSince(ctx, since, limit)` — notifications created at/after `since` (unix seconds), oldest-first, capped at 500; no other filter. Backs `GET /api/notifications?since=<unix>`, the replay pull `myseliasan`'s fleet control plane makes on control-channel reconnect to catch up on notifications this node published while disconnected (see `docs/modules/apps/myseliasan/app/app.go.md`'s "Replay on reconnect"). Delegates to `domain/notification.Service.ListSince`.
  ...
  - `Baseline(ctx, from, to, bucketSeconds, tzOffsetSec, cameraId, source)` — returns `*notification.Baseline`, the expected-activity band (robust median ± k·MAD, Poisson floor) for each bucket of the events-over-time chart, built from the trailing 8 weeks of rollup history. `source` is the new trailing param added for `myseliasan`'s per-node baselines (`domain/entities/notification_rollup.go.md`'s `Source` column); mymatasan always passes `""` (mymatasan has no notion of a source node) — fleet-wide/all-sources behavior is unchanged. Backs `GET /api/notifications/baseline`.
...
- `SaveRecordingConfigRequest` — carries `CameraId`, `Enabled`, `PreRollSec`, `PostRollSec`, `StoragePath`, `RetentionDays`, `SegmentMinutes`, `StreamURL` (recording stream override), `FallbackStreamUrl` (fallback RTSP URI), `MetadataEnabled`/`MetadataGapSeconds` (object metadata recorder toggle + presence-interval close window), `AppearanceEnabled` (W3-2 — turns on appearance descriptors for this camera's sightings; unlike `MetadataEnabled` it is a real choice rather than a consequence of recording, since it costs a forward pass per person/vehicle per sampled frame — see `entities.RecordingConfig`).
