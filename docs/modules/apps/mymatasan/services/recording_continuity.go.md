...
- **Live-benched 2026-08-21 and passing** (15/15, on the shipped defaults — 95% minimum, 2 consecutive bad hours). A containerised install with four cameras: the subject alerted 31 seconds after the hour boundary, on the sweep scoring the SECOND bad hour, and stayed silent after the first; a healthy camera at 100% never alerted; a recording-disabled camera was never scored; the alert fired once rather than once per sweep; both Prometheus gauges were exported; and a camera the health monitor had marked offline reported `reason: "camera-offline"` while a reachable camera on identical 33% coverage reported `unexplained`. The bench is compressible — the monitor scores the previous CLOSED hour on every sweep, so seeding the two hours it is about to score turns "wait two hours" into "wait for one boundary" without weakening a threshold. See `docs/FLAGSHIP_BENCH_CHECKLIST.md`.
- **The disk-guard pause suppression is mutation-tested, not live-benched** — it needs the machine-health guard to actually trip. Removing the `transition == "gap" && paused` check makes `TestContinuityDoesNotAlertWhileTheDiskGuardHasPausedRecording` fail.
- **`alerting` and `lastScoredHour` are in-memory only**, so a restart re-raises an alert for a gap that is still ongoing — one duplicate notification an hour after a restart, on shipped defaults. Accepted: restating an ongoing gap after a restart is defensible, and persisting monitor state would cost a table for it.
