...
- `StartUpdate(ctx)` / background `run()` — errors immediately when `canSelfUpdate()` is false, an update is already applying, or there's no newer version cached; otherwise runs in the background:
  1. Re-fetches the latest release, then `selectAssets` picks the archive matching `runtime.GOOS`/`runtime.GOARCH` (suffix `_<os>_<arch>`, `.zip` on Windows else `.tar.gz`) plus the release's `checksums.txt`.
