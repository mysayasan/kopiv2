...
- Resolve saved devices to RTSP stream URIs through ONVIF media services, then verify the selected stream and persist the working URL.
- Resolve saved devices to ONVIF snapshot URIs for browser MJPEG live view.
- Probe saved or newly selected RTSP URIs through `infra/rtsp`, try selected-profile same-host `/stream1` or `/stream2` fallbacks for cameras such as TP-Link/VIGI, and persist the working URI plus transport/track status.
...
- Live-view preparation preserves an already saved RTSP URI so a working fallback URL is not overwritten by another ONVIF stream resolution.
