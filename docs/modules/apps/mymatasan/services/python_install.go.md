...
- `PythonStatus(ctx)` — reports the AI runtime the app will actually use, not just whether the bundled runtime was downloaded. It probes candidates in priority order via `probeInterpreter` (torch/ultralytics/CUDA via a short `python -c` script): first `vision.detector.command` from the app config (`detectorCommand()` — the interpreter detection really runs with, which may be a user-supplied system Python with its own GPU torch build; resolved via `PATH` when not an absolute path), then the bundled `<dataDir>/pyruntime` interpreter. Returns the first candidate that both exists and has `ultralytics`; if none qualify but at least one candidate exists, returns that one (found, but incomplete) rather than a bare not-found. This keeps a "bring your own Python" `vision.detector.command` setup from being falsely reported as "not installed" merely because the bundled runtime was never downloaded. Returns `PythonRuntimeStatus{Found, Python, Torch, CUDA, Ultralytic}`.
