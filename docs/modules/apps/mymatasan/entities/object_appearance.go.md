...
| `Model`         | string  | The embedder identity string that produced this vector (`"resnet18-hsv-560"`: a 512-d ImageNet embedding concatenated with a 48-d two-band hue/lightness histogram). Vectors from two different models are **never** compared — cosine similarity across unrelated feature spaces returns plausible-looking numbers with no meaning behind them, which is worse than returning nothing because it looks like a result. Every search filters on `Model`, so swapping the embedder degrades to "no older matches" rather than to silent nonsense. |
