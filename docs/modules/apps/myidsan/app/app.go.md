...
- Registers identity, app-registry, user-session, endpoint, RBAC, log, file-storage, and operation-job entities for bootstrap schema generation.
...
- Seeds registered app rows for `myidsan` and `mymatasan`.
- Registers myidsan-local login, user, group, role, and SSO fallback handlers.
