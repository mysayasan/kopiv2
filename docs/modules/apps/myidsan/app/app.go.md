...
- Constructs `sharedservices.NewSetupStateService` (`domain/shared/services`, the suite-wide seam extracted from what used to be a myidsan-local `services.NewSetupStateService` copy) over a `dbsql.NewGenericRepo[sharedentities.RuntimeSetting]` and wires it into `apis.NewSetupApi(api, *deps.Auth, deps.Access, ...)`, mounting `GET /api/setup/state` and `POST /api/setup/complete` (see `apis/setup.go.md`, `domain/shared/services/setup_state.go.md`). The wizard itself (`views/react-webpack/src/views/components/setup.js`) gained a 4th step, "Where sessions live" (`STEP_KEYS` 5 → 6), that reads `GET /api/settings/storage`, offers a live Redis connectivity test (`POST /api/settings/cache/test`), and saves via `PUT /api/settings/storage` by merging its `cache` block onto the live section payload — a naive rebuild would wipe the co-located `fileStorage` block. **This step now also moves `transaction.lockProvider` to `redis` in the same save** (deployment mode / Phase 1 multi-instance safety) and renders the shared `DeploymentPanel` (`frontend/shared/src/Deployment.js`) right below the Redis form — see the deployment-mode bullet below and `apps/myidsan/README.md`.
- Wires the **deployment-mode + cluster-readiness checklist** (deployment mode / Phase 1 multi-instance safety): builds `deploymentModeService := sharedservices.NewDeploymentModeService(runtimeSettingRepo)` and calls `apis.NewDeploymentApi(api, *deps.Auth, deps.Access, deploymentModeService, envFn)` right after the secret cipher is resolved (the checklist's `atrestKey` row needs `secretKeyStore.Fingerprint()` to exist). `envFn` is rebuilt per request (not captured) so a live Settings-editor change to `cache.provider`/`transaction.lockProvider` is reflected immediately. myidsan is one of the two apps in the suite (with `myseliasan`) that can genuinely declare `clustered` — see `domain/shared/services/deployment.go.md`, `apis/deployment.go.md`.
...
- Wires up **Audit log, session administration, and step-up re-authentication**
  ...
  (`/api/audit`, superadmin-only) is also mounted here. `apis.NewStepUpApi`'s call site now
  also passes `loginGuard` — the same `*sharedapis.LoginGuard` instance `apis.NewLoginApi`
  and `apis.NewFederatedAuthApi` share — so `POST /api/step-up` (a password-checking
  endpoint in every sense that matters) is covered by the same lockout the sign-in surfaces
  are, closing a gap where it was the one credential-checking endpoint on the server with no
  throttle behind it. Seeds menu-metadata endpoint rows
  ...
  age-based, archive-first trim of the trail as a periodic `deps.Scheduler` task. Immediately
  after that, calls `consumeMfaResetMarker(deps, mfaService, userLoginService, auditService)`
  (see `app/firstrun.go.md`) — moved here from earlier in boot (before `mfaService` even
  existed to gate on, `auditService` did not exist at all) specifically so the `RESET_MFA`
  escape hatch, which strips the stock superadmin's second factor and deletes its own
  evidence (the factor row and recovery-code hashes), leaves an `mfa.admin_reset` entry in
  the trail instead of only an application-log line.
