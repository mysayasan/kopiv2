...
- `DescribeMetrics(m telemetry.Metrics)` registers help text for all nine; called once at startup
  (`app.go`).

## Sampling, not instrumenting, the ingest gauges

`RunMetricsSampler(ctx, m, stats statSource, devices *DeviceService, interval)` reads
`ingest`'s own atomic counters (`IngestStats`) and device health off a `10s` ticker
(`safego.Supervise`d as `myiotsan.metrics-sampler`) and copies them into gauges — it does **not**
instrument the publish path directly. That path arrives thousands of times a second; taking a
metrics lock on each one would put contention on the exact path the whole app is arranged to keep
fast. The counters are exposed as gauges of the running total, which Prometheus's `rate()` turns
back into a rate — this loses nothing and keeps the sampler a plain copy, one read per interval
instead of one write per reading.

`statSource` is a one-method consumer-defined interface (`Stats() IngestStats`) so the sampler
does not have to import the whole ingest pipeline. Device online/offline counts are derived from
`DeviceService.List` filtered to enabled devices, bucketed on `Health == "offline"`.
...
- Wired from `app.go`'s `RegisterAppRoutes`: `services.DescribeMetrics(deps.Metrics)` then
  ...
  right after `ingest.SetTwin(commandService)`. See `app/app.go.md`.
