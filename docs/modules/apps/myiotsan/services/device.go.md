...
- **(P5)** `CreateDeviceRequest`/`UpdateDeviceRequest` carry `Endpoint`/`Unit`, plumbed straight
  ...
  `services.ModbusPoller` (`modbus_poller.go.md`) to build each device's poll target. They also
  carry `Transport`/`Baud`/`Parity`/`DataBits`/`StopBits` (trimmed the same way `Endpoint` is),
  plumbed onto the matching `entities.IotDevice` fields — which Modbus wire transport (TCP/
  RTU-over-TCP/serial) and, for serial, the line settings (`iot_device.go.md`).
