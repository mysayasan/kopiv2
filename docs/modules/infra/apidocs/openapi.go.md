...
- Strips Gorilla-mux path-parameter regex constraints (`{id:[0-9]+}` → `{id}`) from every emitted path via `stripPathParamPatterns`/`muxPathParam`, so the served document only ever declares bare `{name}` path segments.
...
- `normalizePath` used to carry a mux route's regex constraint straight into the emitted path (e.g. `/api/notifications/{id:[0-9]+}/read`). Because `extractPathParams` already strips at the colon when declaring the operation's path parameters, the emitted path segment and its declared parameter name disagreed — a segment named `id:[0-9]+` with no matching `{id:[0-9]+}` parameter — which made the whole document invalid to strict OpenAPI consumers, not just that one path. OWASP ZAP is the concrete case that surfaced this: it recorded an import error and aborted its automation plan before `activeScan`/`report` ran, so the security scan silently produced nothing. This affects every app that serves `/swagger/openapi.json` (all five apps share this module). Fixed by `stripPathParamPatterns`; regression-covered by `TestBuildSpecStripsMuxRegexFromPathParameters` in `openapi_test.go`.
