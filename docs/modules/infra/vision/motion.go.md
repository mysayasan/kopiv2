...
Provides the first reusable detector implementation: motion detection inside configured rule polygons.
...
- It implements the shared `Detector` interface, so model-backed fire, smoke, person, or vehicle detectors can replace or complement it later.
