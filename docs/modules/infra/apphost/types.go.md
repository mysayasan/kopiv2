...
- `Dependencies`
  ...
  - `Locker` (`coordination.Locker`) — the process's transaction lock, shared (Redis) or per-process (memory) per `transaction.lockProvider`. Used both to serialize work that must happen once for the whole deployment and, via `PingFunc`, to prove reachability on the deployment preflight (`domain/shared/services/deployment.go.md`).
  - `Leader` (`*coordination.Leader`, new — deployment-mode/Phase-1 multi-instance safety) — answers "is this the instance that should run background work?" (`coordination/leader.go.md`). Built once in `run.go` via `coordination.Elect(schedulerCtx, txLocker, ...)` and handed to every app; callers ask `deps.Leader.IsLeader()` inside a task body, not once at registration, so leadership moving takes effect without a restart. With the memory lock provider (the standalone default) the lone process always wins, so gating a task with it changes nothing for a single-instance install.
  - `JwtSecretGenerated` (`bool`, new) — true when no usable `jwt.secret` was configured and the host generated one for this instance and wrote it back to this instance's config file. Config alone cannot reveal this after the fact (the generated value then reads exactly like a configured one), which is why the deployment preflight's `jwtSecret` check needs the host to report it directly rather than inspecting the resolved secret.
