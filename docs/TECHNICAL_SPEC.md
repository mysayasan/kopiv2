...
- **Dashboard Intelligence analytics** (`domain/notification` Heatmap/Baseline/AnomalyScan, DB-backed): an hourly `notification_rollup` table (`RollupMaintainer`, watermark persisted under the `notification.rollup.cursor` runtime-setting key so a restart resumes instead of re-scanning history) incrementally aggregates the notifications feed. `GET /api/notifications/heatmap` returns a local day-of-week × hour-of-day activity grid; `GET /api/notifications/baseline` returns a per-bucket expected-activity band (robust median ± k·MAD, Poisson floor for sparse slots, trailing 8-week lookback) drawn behind the dashboard's events-over-time chart. A statistical anomaly monitor (opt-in, `anomalyDetection` runtime-setting key — `enabled`, `sensitivity` (k, 1–6, default 3), `detectHigh`/`detectLow`, `minActivity`, `requireConsecutive`, `cooldownHours`, `checkIntervalMs`) scores each closed hour per camera against its own (weekday, hour) baseline and raises `analytics.anomaly` category notifications for spikes or "unusual silence" (a normally-active slot going quiet — the tamper/obstruction/offline signal), debounced by consecutive-hour count and a per-camera+direction cooldown; `GET/PUT /api/anomaly/settings` and `GET /api/anomaly/scan` (on-demand preview, independent of the monitor's own debounce state) back the Settings UI.
- **Object metadata recorder** (`recording_config.metadataEnabled`/`metadataGapSeconds`, per camera): logs "what objects each camera saw" as searchable presence-interval rows (`ObjectObservation`), fed by the same object-detector inference the AI rules already run (`vision.ObservationSink`/`ObservationCapable` — no second video decode); a metadata-only camera (no alert rules) still gets exactly one inference pass per sample via `ObserveOnly`. Consecutive sightings of a label coalesce into one interval, closed after `metadataGapSeconds` (default ~5s) of absence. `GET /api/observations` (+ `/labels`) searches presence intervals with `DataTable`-style server-side filter/sort/paging, each result linked to the recording segment covering it for click-to-play. Retention follows each camera's recording `retentionDays` (30-day fallback), purged on the same 6-hourly sweep as recording segments.
