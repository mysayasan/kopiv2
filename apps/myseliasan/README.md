...
- **Authentication** (`login.success`, `login.failure`, `login.lockout`, `password.change`) — added after a live cluster bench (`tools/fleetbench/bench_myseliasan_lockout.py`) found the trail had never recorded a single authentication event: it knew about node adoptions, policy edits and key rotations, so a complete brute-force run against the fleet console left nothing at all in the product's own record of who did what. `login.lockout` is written **once**, on the attempt that engages the lockout, so the event worth alerting on is not buried under every refusal that follows. **The password attempted is never recorded** — a trail that keeps what people typed into a password box is a credential store, and the entries worth having are the near-misses of a real password. The action names match `myidsan`'s so an operator correlating the identity server's trail with the control plane's does not have to learn two vocabularies. On these entries `clientIp` is the **peer** address rather than `X-Forwarded-For`: on a login entry the address is the evidence, a header the caller writes is evidence of nothing, and a forged one would make the trail disagree with the lockout about who was attacking. A proxy's claim is kept in `Metadata` as `claimedForwardedFor`, labelled as claimed rather than observed.
...
`myseliasan` shipped without one. `POST /api/auth/local-login` checked the password and returned, so the only brute-force control on the console that owns every camera in the estate was the generic rate limiter — 30 requests a minute, per source address, per path, **per instance**, refilling forever. That is a budget, not a lockout: it never escalates, never notices one account being attacked from many addresses, and a two-instance cluster simply doubles it. A live bench against a real cluster served **13 consecutive guesses** at ~61ms each and then signed in with the correct password as though nothing had happened.

It now uses the same `sharedapis.LoginGuard` every other app uses, driven by the shared `loginSecurity` config block. That block is **absent** from this app's `config.json` and always has been, which resolves to **enabled** with the suite defaults (8 attempts in 300s, then 60s doubling to at most an hour, plus a 400ms delay on every failure) — the configuration had been promising a lockout the code never implemented.

- **Two keys per attempt**: `ip:<remote addr>` and `user:<lowercased username>`. The source key alone never sees a spray distributed across many addresses, which is the shape credential stuffing actually takes. The tradeoff is deliberate and was measured rather than assumed: someone who knows a username can hold that account locked. That is a nuisance the owner recovers from by waiting; unlimited guessing against a known account is a compromise they do not recover from. Any successful sign-in clears both keys.
- **The address comes from `RemoteAddr`, never `X-Forwarded-For`** — a forwarded header is attacker-controlled, so believing it would let anyone dodge their own lockout by changing a string. A deployment behind a reverse proxy must terminate TLS at this process; that is the documented posture for the whole suite.
- **Deployment-wide.** `myseliasan` is one of the two Tier A clusterable apps, so the guard is attached to the shared cache (namespace `myseliasan`) whenever `cache.provider` is `redis`/`redis-cluster`. Without that, an attacker's budget would be `maxAttempts × instance count` and a locked-out user would simply land on another instance and sign in. The in-memory guard stays as the fallback half, so an unreachable cache degrades to per-process throttling rather than to none.
- **`POST /api/auth/change-password` is behind the same guard.** It re-checks the same account's password, and the bench walked **11 current-password guesses** through it with nothing but a session cookie. A rejected *new* password (too short, unchanged) is a policy refusal and is deliberately **not** counted — otherwise a user could lock themselves out while holding a valid session, trying to pick a password the policy keeps refusing.
- **The refusal is a `429`** carrying the remaining wait both as `Retry-After` and as `retryAfterSeconds` in the body, because a browser cannot read the header without an explicit `Access-Control-Expose-Headers` and a countdown that shows nothing reads as a broken app. The sign-in and change-password screens translate it into all four languages rather than displaying the server's English sentence.
- **The clustering preflight reports it** as a blocker row (`loginLockout`), so an operator declaring a clustered install is told whether the brute-force control is on and whether it is actually shared.
...
## Pre-boot configuration wizard (first install)

Distinct from the in-app **Setup Wizard** below: this one runs *before* the app has a
database or cache handle at all, because the shared apphost reads `db`/`cache`/`server`/
`localAuth` exactly once at boot — an install shipped with the wrong database host or a
squatted port has no in-app screen to fix it, since the app never gets far enough to
serve one. `infra/apphost/firstboot` (see `docs/modules/infra/apphost/firstboot/`) serves
a small embedded 4-language page (en/ms/zh/ar, RTL, theme picker) on its own loopback-only
port (default `127.0.0.1:39530`; `setup.address`/`KOPIV2_SETUP_ADDR` override, an
ephemeral port if that one is taken) in the SAME process, before `config.json` is loaded
for real: database (postgres/mariadb/sqlite, with a live probe reusing the host's own
`newDbCrud`), cache (default/redis, live-probed), the web listen address, and the
bootstrap administrator. On submit it writes `config.json` via the shared
`infra/config/configfile` writer and returns; boot then continues in the same process and
reads the file the wizard just wrote — no restart, no supervisor.

It runs **only** when `deploy/dist/myseliasan-config.json` (the shipped default for a
fresh install) marks itself `"setup": {"completed": false}`, or when an operator sets
`KOPIV2_SETUP=1` on an install that can no longer boot. A config with no `setup` block —
every existing myseliasan install as of this writing — is treated as already configured
and never sees this page; a transient database outage is never a trigger, so a running
fleet control plane that briefly loses its database never gets flipped into a config
wizard. Exposing it beyond loopback (`setup.allowRemote` / `KOPIV2_SETUP_ALLOW_REMOTE=1`)
requires a one-time token in the URL, since the page accepts database credentials and the
administrator password with no authentication of its own. Environment overrides
(`DB_HOST` etc., `applyDbConfigFromEnv`) still win over whatever the wizard wrote.
...
The book has grown to **23 articles** per language across six sections: **Getting started** (what MySeliaSan is, what happens on the first start — the pre-boot configuration wizard above — signing in for the first time, the in-app first-run setup wizard, a tour of the workspace, using and printing this manual), **Fleet** (adopting a node, managing a node, the notification feed, fleet rules, and working inside a node — the embedded node pages, live video, and the crucial point that the *node itself* — not this control plane — decides what a grant lets you do once you're inside), **Map & sites** (the fleet map and its basemap/PMTiles split, buildings and floor plans), **AI agent** (the fleet digest, ask the fleet, setting up a language model), **Reports**, and **Administration** (users/roles/access, the audit log, the settings editor), plus a **Reference** section (troubleshooting, FAQ, glossary) that closes the book. It still trails mymatasan's larger manual, but is no longer the 6-article starter set.
...
curl "http://localhost:3000/api/manual/search?q=adopt+a+node&lang=ms" # ranked BM25 search, section-level
