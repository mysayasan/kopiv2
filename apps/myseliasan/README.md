...
The book is deliberately smaller than mymatasan's 36-article manual: **6 articles** per language across two sections — **Getting started** (what MySeliaSan is, signing in for the first time, the first-run setup wizard, a tour of the workspace, using and printing this manual) and **Fleet** (adopting a node — the fleet key, the claim code, and what to do when discovery finds nothing).
