...
// mypintusan's people-and-badges screen, ported into the control-plane embed. Issuing and
// revoking a badge is routine, reversible and fully logged — operator level on the node, and the
// node's own matrix enforces that for tunnelled callers too.
export function PeoplePage({ onToast }) {
  const t = useT();
  ...
  const columns = [
    { key: 'name', label: t('ndoor.people.name') },
    { key: 'ref', label: t('ndoor.people.ref') },
    { key: 'kind', label: t('ndoor.people.kind'), render: (_v, r) => t(`ndoor.people.kind.${r.kind}`) },
    {
      key: 'status',
      label: t('ndoor.people.status'),
      render: (_v, r) => (
        <span className={`pill pill-${r.status === 'active' ? 'ok' : 'warn'}`}>
          {t(`ndoor.people.status.${r.status}`)}
        </span>
      ),
    },
    {
      key: 'badges',
      label: t('ndoor.people.badges'),
      render: (_v, r) => (
  ...
  return (
    <div className="screen">
      <header className="screen-head">
        <div>
          <h1>{t('ndoor.people.title')}</h1>
          <p className="muted">{t('ndoor.people.subtitle')}</p>
        </div>
        <button type="button" className="btn btn-primary" onClick={() => setAdding(true)}>
          <Ico n="plus" sz={15} /> {t('ndoor.people.add')}
        </button>
      </header>

      {error ? <div className="notice notice-error"><p>{error}</p></div> : null}
      {holders === null && !error ? <p>{t('ndoor.loading')}</p> : null}
      {holders && holders.length === 0 ? <p className="muted">{t('ndoor.people.empty')}</p> : null}
      ...
      {selected ? (
        <Badges holder={selected} onClose={() => setSelected(null)} onToast={onToast} />
      ) : null}
    </div>
  );
}

function AddPerson({ onClose, onSaved, onToast }) {
  const t = useT();
  ...
  return (
    <Modal title={t('ndoor.people.addTitle')} onClose={onClose}>
      <form onSubmit={submit} className="form">
        <label>
          <span>{t('ndoor.people.name')}</span>
          <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
        </label>
        <label>
          <span>{t('ndoor.people.ref')}</span>
          <input required value={form.ref} onChange={(e) => setForm({ ...form, ref: e.target.value })} />
          <small className="muted">{t('ndoor.people.refHint')}</small>
        </label>
        <label>
          <span>{t('ndoor.people.kind')}</span>
          <select value={form.kind} onChange={(e) => setForm({ ...form, kind: e.target.value })}>
            {['staff', 'contractor', 'visitor', 'service'].map((k) => (
              <option key={k} value={k}>{t(`ndoor.people.kind.${k}`)}</option>
            ))}
          </select>
        </label>
        ...
        <p className="muted small">{t('ndoor.people.noAccessYet')}</p>
...
function Badges({ holder, onClose, onToast }) {
  const t = useT();
  ...
  const revoke = async (cred) => {
    const status = window.prompt(`${t('ndoor.badge.revokeTitle')}\nrevoked / lost / stolen / suspended`, 'revoked');
    if (!status) return;
    try {
  ...
  return (
    <Modal title={`${t('ndoor.badge.title')} — ${holder.name}`} onClose={onClose} wide>
      {creds === null ? <p>{t('ndoor.loading')}</p> : null}
      {creds && creds.length === 0 ? <p className="muted">{t('ndoor.badge.empty')}</p> : null}
      {creds && creds.length > 0 ? (
        <table className="mini-table">
          <thead>
            <tr>
              <th>{t('ndoor.badge.format')}</th>
              <th>{t('ndoor.badge.facility')}</th>
              <th>{t('ndoor.badge.number')}</th>
              <th>{t('ndoor.badge.status')}</th>
              <th />
            </tr>
          </thead>
          <tbody>
      ...
      <p className="muted small">{t('ndoor.badge.lostHint')}</p>
...
function IssueBadge({ holder, onClose, onSaved, onToast }) {
  const t = useT();
  ...
  return (
    <form onSubmit={submit} className="form form-inset">
      <label>
        <span>{t('ndoor.badge.format')}</span>
      ...
      <div className="form-row">
        <label>
          <span>{t('ndoor.badge.facility')}</span>
          <input
            type="number"
        ...
        <label>
          <span>{t('ndoor.badge.number')}</span>
      ...
      <small className="muted">{t('ndoor.badge.facilityHint')}</small>

      <div className="form-row">
        <label>
          <span>{t('ndoor.badge.pin')}</span>
          <input type="password" value={form.pin} onChange={(e) => setForm({ ...form, pin: e.target.value })} />
        </label>
        <label>
          <span>{t('ndoor.badge.duressPin')}</span>
          <input
            type="password"
      ...
      <small className="muted">{t('ndoor.badge.duressHint')}</small>
