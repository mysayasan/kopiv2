...
import { accessError, api, forbidden } from './lib/helpers';
import { AccessDenied, ConfirmModal, EmptyState, Field, FormBusyOverlay, Panel } from './ui';
...
// ProfilesPage is the node's device-type catalog: what a class of device reports, where it
...
export function ProfilesPage({ onToast }) {
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setDenied(false);
      setError(accessError(r, t, t('profiles.loadFailed')));
    }
    setBusy(false);
  }, [t]);
  ...
  async function remove(profile) {
    ...
    if (!r.ok) {
      ...
      onToast(accessError(r, t, t('profiles.deleteFailed')), 'error');
      return;
    }
...
function ProfileEditor({ profileId, onBack, onSaved, onToast }) {
  ...
  async function save(e) {
    ...
    const payload = {
      ...
      // node only enforces them on a setpoint.
    ...
    setBusy(false);
    if (!r.ok) {
      setError(accessError(r, t, t('profiles.saveFailed')));
      return;
    }
      ...
      >
        <form className="device-edit-form" onSubmit={save}>
          ...
          {commands.length === 0 ? (
            ...
            // Both min and max at zero is not "unbounded" — the node treats it as an omission
