...
import { accessError, api, forbidden, formatAgo, formatNumber, formatTimestamp } from './lib/helpers';
import { EmptyState, Field, Modal } from './ui';

// Actuation, seen from the control plane.
//
...
//
// Commanding a relay through a FLEET TUNNEL is still commanding a relay. Everything myiotsan
// does locally is kept here, unweakened: the confirm-to-execute dialog, the sent/confirmed
// distinction, and the failure message shown WHOLE. The extra distance between the person and
// the contactor is a reason for more care, not less.
//
// The node owns every gate (read-only by default, admin only, declared commands only,
// server-side bounds, rate limit, no automatic retry) — and over the tunnel the node is the ONLY
// thing that can: it resolves the role name myseliasan asserts against its own matrix. This UI
// therefore does not guess who you are over there. It offers the control, and when the node says
// no, it says so, in the node's own words where it has them.
...
//
// The history and the twin are visible to everyone who can reach the node at all: being able to
// see that somebody unlocked a door is a different power from being able to unlock it, and hiding
// the audit trail from the people who cannot write to it would mean only the people who could
// have done it can see what was done.
export function DeviceControl({ device, onToast }) {
  const t = useT();
  ...
  // The command in flight: the row the node gave back, plus whether we are still waiting for
  // the device to have its say.
  const [issued, setIssued] = useState(null);
  const [waiting, setWaiting] = useState(false);
  const [timedOut, setTimedOut] = useState(false);
  // refusal is a NO from the node — the command was never sent. Either its own validation (a
  // value outside the safe range) or its permission matrix (this grant may not actuate). Its
  // text is the node's, shown exactly as written.
  ...
  const loadCommands = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setError(accessError(r, t, t('cmd.loadFailed')));
    }
    setBusy(false);
  }, [device.id, t]);
  ...
  const loadHistory = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setHistoryError(accessError(r, t, t('cmd.historyFailed')));
    }
  ...
  const loadTwin = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setTwinError(accessError(r, t, t('twin.failed')));
    }
  }, [device.id, t]);
  ...
  useEffect(() => {
    ...
    async function tick() {
      ...
      if (Date.now() >= deadline) {
        // The node has not ruled yet. We do NOT call this a success, and we do not resend.
        setWaiting(false);
        setTimedOut(true);
        loadTwin();
        return;
  ...
  async function send(decl, value) {
    ...
    setConfirming(null);
    if (!r.ok) {
      // A refusal is not a bug to be paraphrased. The node wrote its message to be read by the
      // person who tried — show it, and reload the history, because a refused command is still
      // an audit row ("somebody tried to unlock the front door at 03:00").
      //
      // A 403 is the one refusal the node cannot phrase for this UI, because it is about the
      // caller's grant HERE: it means the role myseliasan asserted for you over the tunnel is
      // not one the node lets actuate. Say that, rather than a bare "Forbidden".
      setRefusal(forbidden(r) ? t('niot.actuateForbidden') : accessError(r, t, t('cmd.issueFailed')));
      setIssued(null);
      setWaiting(false);
      setTimedOut(false);
      loadHistory();
  ...
  // What gates the buttons here is what the NODE has told us about the DEVICE — actuation is
  // switched on for it, and it is enabled — not a guess about who the viewer is. The node
  // resolves the caller's role itself, and a client-side guess that disagreed with it would be
  // worse than the 403: it would either hide a control the operator really has, or dangle one
  // they do not. So the answer comes from the only place that knows it.
  const canIssue = actuationEnabled && !!device.enabled;
  ...
  return (
    <div className="iot-control">
      ...
      {/* The tunnel note. Not a permission guess — a statement of where the decision is made,
          so an operator who is refused knows what to ask for and who to ask. */}
      {!busy && canIssue ? <p className="settings-hint iot-cmd-viewer-note">{t('niot.actuateNote')}</p> : null}
...
function CommandControl({ decl, canIssue, onAsk }) {
  ...
  // A setpoint with no range declared is not "anything goes" — the node refuses EVERY value
      ...
      {kind === 'switch' ? (
        ...
        <div className="iot-cmd-setpoint">
          <Field label={t('cmd.value')} hint={noRange ? '' : t('cmd.range', { min: formatNumber(min), max: formatNumber(max) })}>
            {/* min/max here are a CONVENIENCE, not the enforcement: the node refuses an
...
// wrong door with a stray click. It is kept in the embed for exactly the same reason — a relay
// commanded from the control plane closes just as hard.
...
function IssuedOutcome({ cmd, waiting, timedOut, decl, onDismiss }) {
  ...
  if (cmd.status === 'failed') {
    // The error is the node's, and it is shown WHOLE. The timeout one — "the device never
    ...
    // would throw away the only thing that tells an operator not to just click it again. It is
    // not shortened to fit a fleet UI's chrome; the chrome gives way instead.
    return (
      <Outcome
        kind="failed"
        icon="warning"
  ...
  // Still 'sent' after the whole window: the node has not ruled, so neither do we.
  return (
    <Outcome
      kind="unconfirmed"
      icon="warning"
...
function CommandHistory({ rows, busy, error, declByName }) {
  const t = useT();
  ...
  const columns = [
    ...
    { key: 'requestedBy', label: t('cmd.by'), filterType: 'number', render: (v, row) => actorText(v, row.requestedByName, t) },
...
// actorText names the person. An audit row with no actor is not an audit trail — so an
// unattributed command says so rather than rendering a bare zero.
//
// The NAME is the node's to give. `requestedBy` is a NODE user id, and this page is not the node:
// the myseliasan user reading it is a different account, in a different database, that happens to
// number its rows from 1 too. myiotsan's own copy of this table resolves the id against the
// signed-in session and prints "you"; doing that HERE would eventually label somebody else's relay
// switch as your own — the one thing an audit trail may never do. So we never resolve the id
// ourselves: we print `requestedByName` when the node sends it (it knows who its own ids are, and
// for a tunnelled command it records the operator myseliasan asserted), and otherwise we print the
// id as an id and claim nothing. An older node that does not send the name still reads honestly.
function actorText(id, name, t) {
