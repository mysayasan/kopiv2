...
import { accessError, api, forbidden, formatAgo, formatTimestamp } from './lib/helpers';
import { CredentialModal } from './devices';
import { AccessDenied, ConfirmModal, CopyButton, EmptyState, Field, FormBusyOverlay, Modal, Panel } from './ui';
...
//
// Every route behind this page is admin-only AT THE NODE. Opening a window from the control
// plane opens it on the HUB — the door is at the hub, and so is the broker the device will
// connect to. Nothing here is hidden pre-emptively; if the grant does not stretch this far,
// the node says so and the page shows exactly that.
export function DiscoveryPage({ onToast, host }) {
  const t = useT();
  ...
  const loadStatus = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setError(accessError(r, t, t('disc.statusFailed')));
    }
  }, [t]);
  ...
  const loadCandidates = useCallback(async () => {
    ...
    else if (!forbidden(r)) setError(accessError(r, t, t('disc.candidatesFailed')));
  }, [t]);
  ...
  useEffect(() => {
    if (!status.open || denied) return undefined;
    const id = setInterval(loadCandidates, CANDIDATE_POLL_MS);
    return () => clearInterval(id);
  }, [status.open, denied, loadCandidates]);
  ...
  async function openWindow() {
    ...
    setOpening(false);
    if (!r.ok) {
      onToast(accessError(r, t, t('disc.openFailed')), 'error');
      return;
    }
  ...
  async function closeWindow() {
    ...
    if (!r.ok) {
      onToast(accessError(r, t, t('disc.closeFailed')), 'error');
      return;
    }
    setStatus(r.body || { open: false, expiresAt: 0 });
  ...
  async function adopt(candidate, form) {
    ...
    if (!r.ok) return accessError(r, t, t('disc.adoptFailed'));
  ...
  async function reject(candidate) {
    ...
    if (!r.ok) {
      onToast(accessError(r, t, t('disc.rejectFailed')), 'error');
      return;
    }
  ...
  if (denied) {
    return (
      <section className="workspace">
        <Panel icon="search" title={t('page.discovery')}>
          <AccessDenied />
        </Panel>
      </section>
    );
        ...
        {candidates.length === 0 ? (
          status.open ? (
            <div className="iot-disc-waiting">
              ...
              {/* The address a device must point at is the HUB's, not the control plane's. This
                  page is being read on myseliasan, so window.location.hostname here would send
                  every installer to the wrong machine — the node's own address is the answer,
                  and it is the one thing on this page that had to change in the port. */}
              <p className="settings-hint">{t('disc.emptyOpenSteps', { host: host || t('niot.thisHub') })}</p>
            </div>
          ) : (
...
// The window lengths on offer. The node clamps anything above an hour: a window measured in
...
// device that has just been powered up appears without anybody pressing refresh. It stops the
// moment the window closes — nothing new can arrive after that.
...
// page can re-read the real status from the node rather than deciding for itself that the door
// is shut. (The node enforces the expiry on every connect; this is only the clock the human
// reads.)
...
function CandidateCard({ candidate, profiles, onAdopt, onReject }) {
  ...
  const [form, setForm] = useState(() => ({
    ...
    // type for now"; the node reads 0 as "use the suggestion", which is 0 too.
