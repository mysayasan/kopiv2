...
// PROXY SHIM (myseliasan port): these camera-page components were written for the
// mymatasan app, where apiBase() is same-origin. Here they render inside myseliasan
// against a *managed node*, so every `${apiBase()}/api/...` must route through the
// commander proxy (/api/nodes/{id}/proxy/...). setNodeProxyBase() is called by the
// hosting wrapper before the components render; apiBase() then returns that base so all
// internal GETs (snapshots, frames) tunnel to the node transparently.
let __nodeProxyBase = '';
...
// installProxyCsrf patches window.fetch once so the copied camera-page components — which
// issue their own raw fetch() writes (encoder, stream/live, etc.) without a CSRF header —
// still pass myseliasan's federated-auth CSRF gate when those writes tunnel through the
// commander proxy. It only touches state-changing requests to a /api/nodes/{id}/proxy/
// path; everything else is passed through untouched. Idempotent.
let __csrfPatched = false;
export function installProxyCsrf(tokenProvider) {
  if (__csrfPatched || typeof window === 'undefined' || typeof window.fetch !== 'function') return;
  __csrfPatched = true;
  ...
  window.fetch = (input, init = {}) => {
    try {
      ...
      if (url.includes('/api/nodes/') && url.includes('/proxy/') && method !== 'GET' && method !== 'HEAD') {
        const token = tokenProvider && tokenProvider();
        if (token) {
          const headers = new Headers((init && init.headers) || (typeof input === 'object' && input && input.headers) || {});
          if (!headers.has('X-CSRF-Token')) headers.set('X-CSRF-Token', token);
          init = { ...init, headers };
        }
      }
    } catch (_) { /* never let the shim break a request */ }
