...
// login" option drops in the data dir to request a password reset on next start. It is
// also the documented manual recovery path for a locked-out operator on any platform.
const adminResetMarkerFile = "RESET_ADMIN"

// consumeAdminResetMarker performs the lock-out recovery reset if the marker is present,
// returning the new credential so the caller can announce it. Returns (nil, nil) when
// there is no marker — the normal path.
//
...
func consumeAdminResetMarker(deps apphost.Dependencies, users services.IControlUserService) (*services.StockSeedResult, error) {
	...
	deps.Logger.Warnf("myseliasan.setup", "admin reset marker found — resetting the bootstrap superadmin password")
	seed, err := users.ResetStockSuperadmin(context.Background(), deps.Config.LocalAuth.Username, deps.Config.LocalAuth.Password)
	if err != nil {
		return nil, err
	}
	return &seed, nil
...
// announceFirstRunAdmin surfaces the bootstrap superadmin credential established on a
// fresh install. It writes a recovery file in the data dir — the reliable,
// platform-independent place to find the login, since a Windows service console is
// invisible and a Docker/journal banner scrolls away — and prints a console banner.
//
// The password is echoed only when it was *generated*; a config- or env-supplied one
// is pointed at rather than logged, because the operator already knows it. The account
// is must-change on first sign-in either way. Mirrors mymatasan's first-run banner.
func announceFirstRunAdmin(deps apphost.Dependencies, seed services.StockSeedResult) {
	...
	if !saved {
		deps.Logger.Warnf("myseliasan.setup", "could not write credential recovery file %s", credPath)
	}

	const bar = "======================================================================"
	var b strings.Builder
	fmt.Fprintf(&b, "\n%s\n", bar)
	fmt.Fprint(&b, "  MySeliaSan is ready.\n\n")
	...
	fmt.Print(b.String())
	deps.Logger.Infof("myseliasan.setup", "bootstrap superadmin %q established (must change on next login); sign-in details written to %s", seed.Username, credPath)
}
...
func writeFirstRunCredentialFile(path, url string, seed services.StockSeedResult) error {
	...
	body := "MySeliaSan - one-time administrator login\n" +
		"=========================================\n\n" +
		"Open:      " + url + "\n" +
