...
import { api, errorMessage, formatNumber, formatTimestamp } from '../lib/helpers';
import { Panel, SeverityPill } from './ui';

// AlertsPage is the log: every firing, newest first, with the reading that tripped it.
//
...
// an incident must not be the person who can remove the record of it. The backend does not
// expose a delete either, so this is not a gap in the UI, it is the design.
export function AlertsPage({ onToast }) {
  ...
  useEffect(() => {
    api('/api/devices?limit=1000').then((r) => { if (r.ok) setDevices(r.body?.items || []); });
  }, []);
  ...
  const load = useCallback(async () => {
    ...
    if (r.ok) {
      ...
      setError(errorMessage(r, t('alerts.loadFailed')));
    }
    setBusy(false);
  }, [query, deviceId, t]);
  ...
  async function ack(alert) {
    ...
    setAcking(0);
    if (!r.ok) {
      onToast(errorMessage(r, t('alerts.ackFailed')), 'error');
      return;
    }
