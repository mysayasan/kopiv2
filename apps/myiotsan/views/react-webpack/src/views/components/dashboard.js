...
import { api, errorMessage, formatAgo, formatCount, formatNumber, formatTimestamp } from '../lib/helpers';
import { EmptyState, Panel, SeverityPill } from './ui';

// DashboardPage is the estate at a glance: how many devices are talking, what has fired
// recently, and — the panel an operator learns to read first — what the ingest path is
// doing with everything the sensors send.
export function DashboardPage({ onNavigate }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(!dr.ok ? errorMessage(dr, t('dash.loadFailed')) : '');
    setBusy(false);
  }, [t]);
