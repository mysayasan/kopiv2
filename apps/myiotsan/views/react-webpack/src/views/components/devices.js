...
export function DevicesPage({ onToast, session }) {
  ...
  // never again — the backend keeps only a bcrypt hash of it.
  ...
  const load = useCallback(async () => {
    ...
    if (dr.ok) {
      ...
      setLoadError(errorMessage(dr, t('devices.loadFailed')));
    }
  ...
  async function removeDevice(device) {
    ...
    if (!r.ok) {
      onToast(errorMessage(r, t('devices.deleteFailed')), 'error');
      return;
    }
...
function CreateDeviceModal({ profiles, onClose, onCreated, onToast }) {
  ...
  async function submit(e) {
    ...
    setBusy(false);
    if (!r.ok) {
      // The server owns validation (a duplicate device key, a missing name). Show what it
      // said rather than guessing at it here.
      setError(errorMessage(r, t('devices.createFailed')));
      return;
    }
    ...
    <Modal title={t('devices.addTitle')} onClose={busy ? undefined : onClose} wide>
      <form className="device-edit-form" onSubmit={submit}>
        ...
        <div className="settings-field-grid">
          ...
          {isModbus ? (
            <>
              <Field label={t('devices.endpoint')} hint={t('devices.endpointHint')} required>
                <input value={form.endpoint || ''} onChange={(e) => set({ endpoint: e.target.value })} placeholder="192.168.1.50:502" />
              </Field>
...
function DeviceReadings({ device }) {
  ...
  const loadLatest = useCallback(async () => {
    ...
    if (r.ok) setLatest(r.body || {});
    else setError(errorMessage(r, t('devices.readingsFailed')));
  }, [device.id, t]);
  ...
  // Poll the live values (not the history) so the current-value strip stays current.
  useEffect(() => {
...
function DeviceSettings({ device, profiles, onToast, onChanged, onCredential, onBack }) {
  ...
  async function save(e) {
    ...
    setBusy(false);
    if (!r.ok) {
      setError(errorMessage(r, t('devices.saveFailed')));
      return;
    }
  ...
  async function rotate() {
    ...
    if (!r.ok) {
      onToast(errorMessage(r, t('devices.rotateFailed')), 'error');
      return;
    }
  ...
  return (
    <form className="device-edit-form" onSubmit={save}>
      ...
      <div className="settings-field-grid">
        ...
        {isModbus ? (
          <>
            <Field label={t('devices.endpoint')} hint={t('devices.endpointHint')}>
              <input value={form.endpoint || ''} onChange={(e) => set({ endpoint: e.target.value })} placeholder="192.168.1.50:502" />
            </Field>
            <Field label={t('devices.unit')} hint={t('devices.unitHint')}>
              <input type="number" min="1" max="255" value={form.unit ?? 1} onChange={(e) => set({ unit: Number(e.target.value) })} />
