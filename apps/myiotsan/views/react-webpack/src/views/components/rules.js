...
import { api, errorMessage, formatAgo, formatNumber } from '../lib/helpers';
import { CheckRow, ConfirmModal, Field, FormBusyOverlay, Modal, Panel, SeverityPill } from './ui';
...
export function RulesPage({ onToast }) {
  ...
  const load = useCallback(async () => {
    ...
    if (rr.ok) {
      ...
      setError(errorMessage(rr, t('rules.loadFailed')));
    }
  ...
  async function save(form, id) {
    ...
    if (!r.ok) return errorMessage(r, t('rules.saveFailed'));
  ...
  async function remove(rule) {
    ...
    if (!r.ok) {
      onToast(errorMessage(r, t('rules.deleteFailed')), 'error');
      return;
    }
        ...
        <fieldset className="iot-fieldset">
          ...
          {scope === 'tag' ? (
            <Field label={t('rules.tag')} hint={t('rules.tagHint')}>
              <input value={form.tag} onChange={(e) => set({ tag: e.target.value })} list="iot-rule-tags" />
            </Field>
          ) : null}
          <datalist id="iot-rule-tags">
          ...
          <div className="settings-field-grid">
            ...
            {fields.key ? (
              <Field label={t('rules.key')} hint={t('rules.keyHint')} required>
                <input value={form.key} onChange={(e) => set({ key: e.target.value })} list="iot-rule-keys" />
              </Field>
            ) : null}
          </div>
          <datalist id="iot-rule-keys">
            ...
            {fields.window ? (
              <Field label={t('rules.window')} hint={t(`cond.${form.condition}.windowHint`)} required>
                {/* Deliberately no `min` constraint. A native min would make the BROWSER
                    ...
                    so the value goes to the server and the server's answer is shown. */}
                <input
                  type="number"
