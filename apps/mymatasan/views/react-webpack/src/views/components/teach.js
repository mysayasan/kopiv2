    ...
    <section className="settings-panel">
      ...
      {isActive ? (
        ...
        <>
          ...
          <header className="training-divider"><h3>{t('teach.testDriveTitle')}</h3></header>
          <p className="settings-hint">{t('teach.testDriveLead')}</p>
          {!driving ? (
            <div className="action-row">
              <button type="button" className="quiet" onClick={startDrive} disabled={busy}>
                <span className="btn-icon"><Ico n="play" /> {t('teach.startTestDrive')}</span>
              </button>
            </div>
          ) : (
            <>
              {drive && drive.ready && drive.image ? (
                <img className="teach-drive-frame" src={`data:image/jpeg;base64,${drive.image}`} alt={t('teach.testDriveTitle')} />
              ) : (
                <span className="teach-live-row"><span className="teach-live-dot" /> {t('teach.warmingUp')}</span>
              )}
              {drive?.error ? <span className="field-hint danger-text">{drive.error}</span> : null}
              {drive && drive.ready ? (
                <span className="field-hint">{seeing ? t('teach.seeing', { what: seeing }) : t('teach.seeingNothing')}</span>
              ) : null}
              <div className="action-row">
                <button type="button" className="quiet" onClick={stopDrive} disabled={busy}>
                  <span className="btn-icon"><Ico n="stop" /> {t('teach.stopTestDrive')}</span>
                </button>
              </div>
            </>
          )}
