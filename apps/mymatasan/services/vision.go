...
func (s *visionService) GetAlerts(ctx context.Context, limit uint64, offset uint64) ([]*entities.AlertEvent, uint64, error) {
	sorters := []sqldataenums.Sorter{{FieldName: "CreatedAt", Sort: sqldataenums.DESC}}
	return s.alerts.Get(ctx, "", limit, offset, nil, sorters)
}
