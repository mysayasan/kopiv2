...
- **Per-IP failed-login lockout** now covers every credential surface (local login, LDAP login, the server-rendered federated login page) via the shared `loginSecurity` config — previously myidsan had none.
