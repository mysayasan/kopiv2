...
**Shared footer**: a `AppFooter` component (`@shared`) renders at the bottom of the app shell, showing the app name, version, shared-core version, short commit hash, build date (fetched from `/api/version`), and the KopiV2 product tagline. All fields are optional and degrade gracefully if the endpoint is unreachable.
