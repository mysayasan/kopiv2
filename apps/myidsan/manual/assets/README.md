...
- **Nothing loaded from the network.** These files are compiled into the binary, which is what
  keeps the manual working with no outbound connection at all — the posture the whole suite is
  built for. An article must never reference an external image URL.
