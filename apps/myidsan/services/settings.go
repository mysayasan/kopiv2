...
//
// Persistence model (decided against a pure DB store because myidsan's config is
...
var sectionOrder = []string{"localAuth", "sso", "security", "storage", "logging"}

// DELIBERATELY NOT EDITABLE HERE, beyond the db/server/bootstrap blocks myseliasan also
// withholds:
//
//   - audit.retention. The security trail's one removal path is config-file-only ON
//     PURPOSE, so trimming it needs filesystem access to the server rather than a session
//     on it — the superadmin whose own actions the trail records cannot reach for it from
//     inside the product. Surfacing it in this editor would hand back exactly the capability
//     that design withholds. See apps/myidsan/services/audit_retention.go.
//   - kerberos. Enabling it requires placing a keytab on the host anyway, so it is already
//     a filesystem-level setup step; a half-set keytab path here would only turn the SSO
//     button into a broken one.
//   - login.oidc[] and the social providers. These carry client secrets per provider and
//     are better handled by the dedicated Apps/Federation screens than a generic form.
...
var sectionSecrets = map[string][]string{
	...
	// internalToken authenticates server-to-server introspection calls; it is a bearer
	// credential, not a setting, so it is masked like any other secret.
	"sso":      {"sso.internalToken"},
	"security": {"jwt.secret"},
...
func NewSettingsService(cfg *config.AppConfigModel, cfgPath string, db dbsql.IDbCrud, cipher *atrest.Cipher, logf func(format string, args ...any)) ISettingsService {
	...
	// A nil database disables only the defaults snapshot, and therefore Reset. Editing and
	// saving still work, because those go to config.json rather than the database. Guarded
	// rather than assumed non-nil so the editor is constructible (and testable) without one.
	if db != nil {
		s.repo = dbsql.NewGenericRepo[sharedentities.RuntimeSetting](db)
...
func (s *settingsService) read(section string) (map[string]any, error) {
	...
	case "sso":
		// The shared SSO block carries both sides of the federation. myidsan is the
		// PROVIDER, so only the issuing fields are exposed; providerBaseUrl / clientId /
		// clientSecret / redirectBaseUrl / redirectPath describe a RELYING app pointing at
		// an IdP and mean nothing on the IdP itself.
		//
		// The TTLs are the operationally interesting part: authCode is deliberately short
		// (it is exchanged immediately), while session governs how long a federated login
		// stays valid across every relying app — raising it raises the window a stolen
		// cookie is useful for, on every app at once.
		return map[string]any{"sso": map[string]any{
			...
			"authCodeTtlSeconds":    s.cfg.SSO.AuthCodeTTLSeconds,
			"accessTokenTtlSeconds": s.cfg.SSO.AccessTokenTTLSeconds,
			"policyCacheTtlSeconds": s.cfg.SSO.PolicyCacheTTLSeconds,
			"internalToken":         s.cfg.SSO.InternalToken,
		}}, nil
	case "security":
		return map[string]any{
			...
			// The three identity policies. These are the reason a Settings page is worth
			// more on myidsan than on the other apps: they are absent from the shipped
			// config.json entirely and resolve through Effective*() defaults, so today an
			// operator cannot see that the lockout is ON, what the password floor is, or
			// what the MFA policy resolves to — let alone change any of it without editing
			// a file. Read through the resolvers so the form shows the values actually in
			// force rather than the zero values of an absent block.
			"loginSecurity":  effectiveLoginSecurityMap(s.cfg.LoginSecurity.Effective()),
			"passwordPolicy": effectivePasswordPolicyMap(s.cfg.PasswordPolicy.Effective()),
			"mfa":            effectiveMfaPolicyMap(s.cfg.Mfa.Effective()),
		}, nil
	case "storage":
		return map[string]any{
...
func (s *settingsService) ensureDefaults(ctx context.Context) error {
	...
	now := time.Now().Unix()
	_, err = s.repo.Create(ctx, "", sharedentities.RuntimeSetting{Key: settingsDefaultsKey, Value: stored, CreatedAt: now, UpdatedAt: now})
	return err
}
...
// The three resolver->map helpers below exist so the editor shows the values IN FORCE.
// Each of these blocks is optional in config.json and resolves through an Effective*()
// function, so reading the raw struct would show zeros for an absent block and invite an
// operator to "fix" a lockout that was already on.
func effectiveLoginSecurityMap(e config.EffectiveLoginSecurity) map[string]any {
	return map[string]any{
		"enabled":           e.Enabled,
		"maxAttempts":       e.MaxAttempts,
		"windowSeconds":     e.WindowSeconds,
		"lockoutSeconds":    e.LockoutSeconds,
		"lockoutMaxSeconds": e.LockoutMaxSeconds,
		"failedDelayMs":     e.FailedDelayMs,
		"notifyOnLockout":   e.NotifyOnLockout,
	}
}

func effectivePasswordPolicyMap(e config.EffectivePasswordPolicy) map[string]any {
	return map[string]any{
		"minLength":     e.MinLength,
		"requireUpper":  e.RequireUpper,
		"requireLower":  e.RequireLower,
		"requireDigit":  e.RequireDigit,
		"requireSymbol": e.RequireSymbol,
		"blockCommon":   e.BlockCommon,
	}
}

func effectiveMfaPolicyMap(e config.EffectiveMfaPolicy) map[string]any {
	roles := e.RequiredRoleIds
	if roles == nil {
		roles = []int64{}
	}
	return map[string]any{
		...
		"applyToDirectory": e.ApplyToDirectory,
	}
}
