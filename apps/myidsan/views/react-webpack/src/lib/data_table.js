...
// ClientDataTable is the self-contained, filterable/sortable grid (mirrors
// myseliasan's data_table.js) used for inline-action pages like Users — where the
// generic CrudPage's modal editor isn't wanted. Filtering/sorting/paging run
// client-side over the supplied rows; it reuses myidsan's existing table CSS classes.
...
export function ClientDataTable({ rows, columns, pageSize = 10, busy = false, emptyText = 'No records' }) {
  ...
  const fieldColumns = useMemo(
    () => columns.filter(c => c.filterable !== false).map(c => ({ ...c, filterType: c.filterType || inferFilterType(c.key) })),
    [columns]
  )
  ...
  const filters = useMemo(
    () => fieldColumns.flatMap(column => normalizeFilterDrafts(columnFilters[column.key], column)
      .map(filter => {
        ...
        return { fieldName: column.key, compare: normalizeFilterCompare(filter?.compare, column), value: coerceFilterValue(value, column) }
      })
      .filter(Boolean)),
  ...
  const openColumnFilter = (column, anchor) => {
    ...
    setOpenFilter({ column, left: Math.max(12, Math.min(rect.left, window.innerWidth - 300)), top: rect.bottom + 8 })
  }
          ...
          <div className="filter-condition" key={`filter-${column.key}-${index}`}>
            ...
            <label>
              ...
              {column.filterType === 'boolean' ? (
                ...
                <input autoFocus={index === 0} value={item.value} onChange={e => updateDraft(index, { value: e.target.value })} type={column.filterType === 'number' ? 'number' : 'text'} />
              )}
            </label>
          </div>
        ))}
...
function inferFilterType(key) {
  if (['isActive', 'isSuperadmin', 'builtin'].includes(key)) return 'boolean'
