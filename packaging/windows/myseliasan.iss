...
// On uninstall, offer to remove the writable data root (database, settings, at-rest key).
// The default is to KEEP it. That matters more here than on a node: the at-rest key
// encrypts the fleet CA key and pairing PSK, so destroying it means re-adopting every
// node. A tester wanting a clean first-run experience just answers Yes.
...
begin
  ...
  begin
    ...
    if UninstallSilent then
      Exit;
    if MsgBox('Also delete all MySeliaSan data — the database, users, settings and the' + #13#10 +
