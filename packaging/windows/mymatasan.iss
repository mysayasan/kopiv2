...
// (deploy/nfpm/postremove.sh), so one variable means the same thing on both platforms.
...
// On uninstall, decide what happens to the writable data root (recordings, database,
...
// prompts. Mirrors `mymatasan-uninstall --purge-data` on Linux.
  ...
  begin
    ...
    begin
      if UninstallSilent then
        Exit;
      wipe := MsgBox('Also delete all MyMataSan data — recordings, the database, settings' + #13#10 +
                     'and the encryption key — under' + #13#10 + dataDir + '?' + #13#10 + #13#10 +
                     'Recordings you pointed at another drive are not affected.' + #13#10 + #13#10 +
