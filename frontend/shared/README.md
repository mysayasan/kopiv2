...
- `ManualProvider` / `ManualLibrary` / `ManualDrawer` / `HelpButton` / `useManual` — the built-in user manual. The app supplies only CONTENT (markdown embedded in its binary and served by `/api/manual`, see `domain/shared/manual`); this module supplies the reader, search, the contextual slide-over, and the printed book. Wiring an app is: mount `<ManualProvider apiBase={apiBase()} lang={lang} appName="…">` around the tree (OUTSIDE the authenticated shell, so help works on the sign-in screen and in the first-run wizard), render `<ManualLibrary/>` on a Help page, and call `useManual().openHelp(slug, anchor)` from a `?` button. Ships its own tokenized CSS (`styles/manual.css`) including the `@media print` rules — "Print the whole manual" renders every article, with a table of contents, into a portal on `<body>` and calls `window.print()`, so Save-as-PDF works in every language (a server-side PDF could not: `domain/report`'s pure-Go writer is cp1252-only and cannot render Chinese or Arabic).
  ...
  - Search ranks against the server's `GET /api/manual/search?q=&lang=&limit=` (`domain/shared/apis/manual.go.md`) — BM25 over the article's own structure, returning the matching **section** rather than the whole article — and falls back to the old client-side substring pass only when that request is missing or fails, so a not-yet-updated app degrades to a worse search rather than none.
  - `src/manual/diagram.js` renders `` ```flow ``/`` ```arch ``/`` ```seq ``/`` ```spec `` fences to inline SVG (a `` ```spec `` fence renders as a `<table>` instead): `--ui-*` tokens only, RTL-aware layout mirroring (Latin/code values inside a figure stay LTR), no script so it survives `window.print()`, and drawn at natural size with sideways scroll rather than shrunk to fit. The grammar it draws is owned in Go by `domain/shared/manual/diagram` and enforced at build time by `domain/shared/manual/manualcheck`'s Diagrams check, so this renderer is deliberately the lenient half — see `docs/MANUAL_DIAGRAMS.md`.
  - A long article renders an "on this page" outline rail (`ManualOutline`) from its own heading structure, tracking scroll position to highlight the section currently in view.
